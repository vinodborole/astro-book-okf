...
Props that are passed to interactive framework components [using a  client:* directive](/en/reference/directives-reference/#client-directives) must be 

[serialized](https://developer.mozilla.org/en-US/docs/Glossary/Serialization): translated into a format suitable for transfer over a network, or storage. However, Astro does not serialize every type of data structure. Therefore, there are some limitations on what can be passed as props to hydrated components.
...
You can, however, use [the Astro  <slot /> pattern](/en/basics/astro-components/#slots) to pass static content generated by Astro components as children to your framework components 

**inside an**.

`.astro` component## Can I hydrate Astro components?
...
`<script>` tags in Astro components[Contribute](/en/contribute/)
